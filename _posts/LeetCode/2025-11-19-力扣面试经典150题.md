...
![](/img/in-post/leetcode/classical-150/21-title.png)
...
![](/img/in-post/leetcode/classical-150/21-res.png)
...
**示例 1：**
![](/img/in-post/leetcode/classical-150/138-title1.png)
```
输入：head = [[7,null],[13,0],[11,4],[10,2],[1,0]]
输出：[[7,null],[13,0],[11,4],[10,2],[1,0]]
```
**示例 2：**
![](/img/in-post/leetcode/classical-150/138-title2.png)
```
输入：head = [[1,1],[2,1]]
输出：[[1,1],[2,1]]
```
**示例 3：**
![](/img/in-post/leetcode/classical-150/138-title3.png)
...
这道题最麻烦的是random的节点的复制，由于它的随机指向性，我们必须在通过next指针完成一次的复制之后再为新数组的random赋值。为了节省时间，可以借助HashMap来记录新旧链表中节点的映射关系，这样通过HashMap以及旧链表的random指针，我们就可以很方便地得知新链表的random指针应该具体指向哪一个节点。用Python代码实现如下
...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        while current is not None :
            new_current = Node(x=current.val)
            node_dict[current] = new_current
            new_last.next = new_current
        ...
        while current is not None:
            if current.random is not None:
                new_current.random = node_dict[current.random]
            current = current.next 
            new_current = new_current.next 
        return new_head 
```
![](/img/in-post/leetcode/classical-150/138-res.png)
...
**示例 1：**
![](/img/in-post/leetcode/classical-150/92-title.png)
```
...
先找到反转开始的左节点，以及左节点左边的节点，然后逐个开始反转，到达右节点时停止反转，将右节点赋给左节点上一个节点的next，左节点的next设置为右节点的next即可，但需要注意实现的细节，具体Python代码实现如下
...
![](/img/in-post/leetcode/classical-150/92-res.png)
...
![](/img/in-post/leetcode/classical-150/19-title.png)
```
...
最简单的方法就是两次遍历，先得知整个数组的长度，然后计算出倒数第N个节点是整数第几，然后再次遍历数组，将这个节点删掉，Python代码实现如下
...
令我意外的是，这种两次遍历的方法在LeetCode的提交记录中耗时竟然是最低的

![](/img/in-post/leetcode/classical-150/19-res.png)
...
**示例 1：**
![](/img/in-post/leetcode/classical-150/82-title.png)
```
输入：head = [1,2,3,3,4,4,5]
输出：[1,2,5]
```
**示例 2：**
![](/img/in-post/leetcode/classical-150/82-title2.png)
```
输入：head = [1,1,1,2,3]
输出：[2,3]
``` 
...
对于每个节点可以通过判断与前一个节点是否相同，以及与后一个节点是否相同来确定是否保留。但是开头和结尾处需要注意：

- 开头处可以通过增加一个虚拟节点来处理
- 结尾处需要注意当最后一个节点不符合要求时要主动删去，我在第一次提交时就是因为忽视了这一点而未通过
...
class Solution:
    def deleteDuplicates(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        pre_head = ListNode(val=-911, next=head)
        last_head = pre_head
        last_node = pre_head 
        current = last_node.next 
        while current.next is not None:
            if last_node.val != current.val and current.val != current.next.val:
                last_head.next = current 
                last_head = current 
            last_node = current 
            current = current.next 
        if last_node.val != current.val:
            last_head.next = current 
        else:
            last_head.next = None
        return pre_head.next
...
![](/img/in-post/leetcode/classical-150/82-res.png)

另外，由于通过提示可知节点数量不多，并且这个链表是排好序的，我们也可以先统计每个取值的数量，然后再次遍历链表只保留符合要求的节点
...
**示例 1：**
![](/img/in-post/leetcode/classical-150/61-title.png)
...
**示例 2：**
![](/img/in-post/leetcode/classical-150/61-title2.png)
```
输入：head = [0,1,2], k = 4
输出：[2,0,1]
 ```
...
关键是要找到新的头结点和尾结点，并且要注意k这个数是有可能取值比较大的，因而应该先取模，再旋转。用Python代码实现如下
...
提交结果如下所示
![](/img/in-post/leetcode/classical-150/61-res.png)
...
**示例 1：**
![](/img/in-post/leetcode/classical-150/86-title.png)
...
可以创建两个链表，一个表示值小于x的所有节点，另一个存储所有大于等于x的节点，然后将两个链表拼接起来就可以了，总的时间复杂度为 $ O(n) $，同时因为大部分的链表操作为原址操作，所以额外的空间复杂度为 $ O(1)$，用Python代码实现如下
```python
# Definition for singly-linked list.
# class ListNode:
#     def __init__(self, val=0, next=None):
...
class Solution:
    def partition(self, head: Optional[ListNode], x: int) -> Optional[ListNode]:
        if head is None or head.next is None:
            return head 
        small_head = ListNode(val=0)
        large_head = ListNode(val=0)
        ...
        while current is not None:
            ...
            current = current.next 
        small_current.next = large_head.next 
...
```
提交结果如下
![](/img/in-post/leetcode/classical-150/86-res.png)
...
这道题的难度来源于要求get和put操作都是 $O(1)$ 的时间复杂度，单纯靠一个基础数据结构貌似难以实现。首先对于get操作，如果要达到 $O(1)$的复杂度，最容易想到的时间复杂度就是HashMap，但是还要实现容量的管理，以及最新的操作过的元素日期更新，长时间未操作的元素过期等功能，显然是HashMap无法单独实现的。对此我想到了再增加一个双向链表进行管理。每次访问时用HashMap快速定位到目标节点，同时在链表中将这个节点放到链表的尾部。
这个代码实现起来颇为复杂，各种分支条件、边界条件都需要仔细编写，如果在真实面试场景中遇到这道题，很可能会写不完。

```python
class Node:
    def __init__(self, key, value=0, next=None, alive=True):
        self.key = key
        self.value = value 
        self.next = next
        self.last = None 
...
class LRUCache:
    ...
    def get(self, key: int) -> int:
        if key not in self.map_dict:
            return -1
        if not self.map_dict[key].alive:
            return -1
        node = self.map_dict[key]
        res = node.value
        # 在size>1时把这个节点放到最后
        if self.size == 1:  # 只有一个节点时不需要操作
            return res
        if node.last is None:  # 如果是头结点
    ...
    def put(self, key: int, value: int) -> None:
        if key not in self.map_dict:  # 新节点
            # print('新节点')
            node = Node(key=key, value=value)
            self.map_dict[key] = node
            if self.size > 0:
                # print('缓存不为空')
                self.tail.next = node
                node.last = self.tail
                ...
                if self.size == self.capacity:  # 如果满了需要使头结点移动
                    # print('满了，删除头')
                    self.head.alive = False
                    # self.map_dict[self.head.key].alive = False
                    self.head = self.head.next
                    self.head.last = None
                else:
                    # print('不满，直接标记空间+1')
            ...
            node = self.map_dict[key]  # 移动到最后
            node.value = value
            if node.alive:  # 节点还存活
                if node.last is None:  # 说明是头结点
                    if self.size > 1:
                        self.head = node.next
                        self.head.last = None
                else:
                    if node.next is not None:
                        node.last.next = node.next
                        node.next.last = node.last
                if self.size > 0 and node.next is not None:
                    self.tail.next = node
                    node.last = self.tail
                self.tail = node
                node.next = None
...
![](/img/in-post/leetcode/classical-150/146-res.png)
...
![](/img/in-post/leetcode/classical-150/104-title.png)
...
![](/img/in-post/leetcode/classical-150/104-res.png)
...
**示例 1：**
![](/img/in-post/leetcode/classical-150/100-title1.png)
```
输入：p = [1,2,3], q = [1,2,3]
输出：true
```
...
**示例 2：**
![](/img/in-post/leetcode/classical-150/100-title2.png)
```
输入：p = [1,2], q = [1,null,2]
输出：false
```
...
**示例 3：**
![](/img/in-post/leetcode/classical-150/100-title3.png)
```
输入：p = [1,2,1], q = [1,1,2]
输出：false
 ```
...
可以使用递归方法，先判断当前节点是否相同，如果相同则继续判断左右孩子是否相同，用Python实现如下
...
提交结果如下
![](/img/in-post/leetcode/classical-150/100-res.png)
...
**示例 1:**
![](/img/in-post/leetcode/classical-150/105-title.png)
```
...
根据前序遍历确定根节点的值，然后到中序遍历中找到这个头结点，在中序遍历中头结点左边的均属于左子树，右边的都属于右子树。由此确定左子树与右子树的大小，如此即可写出递归的解法
...
class Solution:
    def buildTree(self, preorder: List[int], inorder: List[int]) -> Optional[TreeNode]:
        ...
        if left_size > 0:
            root.left = self.buildTree(preorder[1:left_size+1], inorder[0:left_size])
        if right_size > 0:
            root.right = self.buildTree(preorder[len(preorder)-right_size:], inorder[root_index+1:])
        return root
...
![](/img/in-post/leetcode/classical-150/105-res.png)

在有些语言中，如果因为递归导致需要copy数组，其实也可以通过为每个数组添加两个指针来标记范围以实现节约空间的目的。
这种实现方法，每次在中序遍历数组中查找根节点所需时间是 $ O(n) $ 的，大致需要 $ \text{log} n $级别次，所以时间复杂度为 $ O(n\text{log}n) $

为了加速，在官方解法中使用了一个HashMap来记录中序遍历中每个节点的位置，这样在中序遍历数组中搜索根节点的时间复杂度就可以从 $ O(n) $ 降低至 $ O(1) $。
...
在已经排好序的数组或矩阵中搜索目标值，典型的二分查找，用Python实现如下
    ...
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        if n == 1:
            ...
            if matrix[0][mid] == target:
                ...
                return self.searchMatrix([matrix[0][0: mid]], target)
        else:
            n2 = n // 2
            if matrix[n2][0] == target:
                return True 
            elif matrix[n2][0] < target:
                return self.searchMatrix(matrix[n2:], target)
            else:
                return self.searchMatrix(matrix[0:n2], target)
        
```

提交结果如下所示

![](/img/in-post/leetcode/classical-150/74-res.png)
...
可以通过一个循环，记录s中从头开始到哪个位置可以被字典中的单词表示，如果到最后一个位置也可以表示出来，那么就是整个字符串s均可以被字典中的单词表示出来。用Python实现如下

```python
class Solution:
    def wordBreak(self, s: str, wordDict: List[str]) -> bool:
        records = [0 for _ in range(0, len(s))]
        ptr = 0
        while ptr < len(s):
            for w in wordDict:
                lw = len(w)
                if ptr+lw<=len(s) and s[ptr:ptr+lw]==w:
                    records[ptr+lw-1] = 1
            go_on = False
            ...
            if not go_on:
                return False 
            elif go_on and ptr == len(s):
                return True 
```

提交结果如下

![](/img/in-post/leetcode/classical-150/139-res.png)
...
class Solution:
    def coinChange(self, coins: List[int], amount: int) -> int:
        if amount == 0:
            return 0 
        records = [100000 for _ in range(0, amount+1)]
        for c in coins:
            if c <= amount:
                records[c] = 1
        for c in coins:
            ...
            for i in range(c, amount+1):
                records[i] = min(records[i], records[i-c]+1)
        if records[amount]<100000:
...
![](/img/in-post/leetcode/classical-150/322-res.png)
...
进阶里要求的 $ O(n \text{log}(n)) $ 时间复杂度的做法暂时没有想到，想到了一个 $ O(n^2) $时间复杂度的做法：

每个严格递增子序列必定会以某个元素结尾，假设以 nums[i] 结尾，则这个子序列的最大长度必定为 i 前面的某个小于 nums[i] 的元素结尾的子序列的长度再加1，如果前面没有比 nums[i] 小的元素，那么以 nums[i] 结尾的递增子序列的长度为 1 。最后我们遍历所有记录数组，找出长度最长的子序列长度就可以了。因为是两层嵌套循环，时间复杂度为  $ O(n^2) $，用Python实现如下
...
class Solution:
    def lengthOfLIS(self, nums: List[int]) -> int:
        if len(nums) == 1:
            return 1
        records = [1 for _ in range(0, len(nums))]
        for i in range(1, len(nums)):
            for j in range(0, i):
                if nums[i] > nums[j]:
...
![](/img/in-post/leetcode/classical-150/300-res.png)
...
class Solution:
    def minimumTotal(self, triangle: List[List[int]]) -> int:
        ...
        for i in range(1, len(triangle)):
            index = len(triangle) - 1 - i 
            for j in range(0, len(triangle[index])):
                triangle[index][j] = min(triangle[index+1][j], triangle[index+1][j+1]) + triangle[index][j]
        return triangle[0][0]
```

提交结果如下

![](/img/in-post/leetcode/classical-150/120-res.png)
...
动态规划，依次判断从每个位置开始长度为 step 的子串是否为回文，最后遍历所有记录，找出最长的那个子串。用Python实现如下
...
class Solution:
    def longestPalindrome(self, s: str) -> str:
        if len(s) <= 1:
            return s 
        n = len(s)
        records = [[0 for _ in range(0, n)] for __ in range(0, n)]
        for i in range(0, n):
            records[i][i] = 1
        for step in range(1, n):
            for i in range(0, n):
                ...
                if s[i] == s[i+step]:
                    if step == 1:
                        ...
                        if records[i+1][i+step-1] == 1:
                            records[i][i+step] = 1
        for step in range(n-1, -1, -1):
            for i in range(0, n):
                if step + i >= n:
                    break 
                if records[i][i+step]==1:
                    return s[i:i+step+1]
        return s[0]
```
提交结果如下
![](/img/in-post/leetcode/classical-150/5-res.png)
...
class Solution:
    def canConstruct(self, ransomNote: str, magazine: str) -> bool:
        ...
        for i in range(0, len(magazine)):
            if magazine[i] not in alpha_count:
                alpha_count[magazine[i]] = 1
            else:
                alpha_count[magazine[i]] += 1
        for i in range(0, len(ransomNote)):
            if ransomNote[i] not in alpha_count:
                return False 
            alpha_count[ransomNote[i]] -= 1
            if alpha_count[ransomNote[i]] < 0:
                return False 
        return True
...
![](/img/in-post/leetcode/classical-150/383-res.png)
...
我们知道，在排好序的数组中搜索指定数的时间复杂度是 $O(\text{log}n)$的，这道题的不同之处在于数组是经过一个旋转操作的，带来的困难是我们无法简单的仅通过开头位置和结尾位置来判断目标值是否有可能不存在。但其实，我们仍然可以直接将数组分为两半，则至少有一个子数组是严格排好序的，如图判断呢？只需要根据子数组的开头和结尾就可以判断，加入开头位置大于结尾位置，说明子数组内部存在旋转，否则说明子数组已经是严格排好序的。所以这个问题的时间复杂度仍然是 $O(\text{log}n)$的。
...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        left = 0 
        right = len(nums) - 1
        return self.search_range(nums, target, left, right)

    def search_range(self, nums: List[int], target: int, left: int, right: int):
        if left > right:
            return -1
        if nums[left] == target:
            return left 
        ...
        if right - left <= 1:
            return -1 
        if nums[left] < nums[right] and (nums[left] > target or nums[right] < target):
            return -1
        else:
            # 二分法查找
            mid = (right + left) // 2
            if nums[mid] == target:
                return mid 
            res1 = self.search_range(nums, target, left+1, mid-1)
            res2 = self.search_range(nums, target, mid+1, right-1)
...
![](/img/in-post/leetcode/classical-150/33-res.png)